...
## Migration guides
- [MIGRATING_0.29_TO_0.30.md](MIGRATING_0.29_TO_0.30.md) — flowscope 0.24, the `Http2` marker (signature dispatch — widens the kernel prefilter and probes every TCP flow), `MonitorBuilder::capture_in_netns` + `Monitor::capture_sources` (#135; flow keys still alias across namespaces), and one break: the publicly re-exported `etherparse` 0.16 → 0.21.
