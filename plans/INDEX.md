...
| Plan | Theme |
|---|---|
| [`netring-arp-plan.md`](./netring-arp-plan.md) | **ARP** ✅ **MERGED to master** — issues **#12 + #19 + #20 all closed** (PRs #18/#22/#21). `arp` feature: `on_arp`/`on_arp_anomaly` (spoof/binding-change) parsed per-frame in the drain over flowscope 0.17's `arp` module + `NeighborTable`; `Ctx::arp_table()` (ARP-drain exposure); `Atom::EtherType` kernel-pushdown (ARP no longer forces capture-all). Ops-toolkit hooks, not `MessageProtocol` (ARP has no 5-tuple). Remaining tail in plan §7 (cross-protocol `arp_table` threading; `session::<Arp>()` field gating; NDP sibling). Delete-on-ship at the next release. |
